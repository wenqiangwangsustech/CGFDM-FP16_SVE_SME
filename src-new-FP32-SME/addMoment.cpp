...
__GLOBAL__
void calculate_MomentRate( long long npts, long long nt, FLOAT * CJM, float *  momentRate, long long * srcIndex, GRID grid )
{
...
void calculateMomentRate( SOURCE_FILE_INPUT src_in, FLOAT * CJM, float * momentRate, long long * srcIndex, GRID grid )
{
	...
	<<< blocks, threads >>>
	( npts, nt, CJM, momentRate, srcIndex, grid );
				...
				int _nx_, int _ny_, int _nz_, int flagSurf, float DT )
{
	...
	CALCULATE1D( i, 0, npts )
		...
		hW[idx+3*num] -= momentRateSlice[i*MOMSIZE+0] * factorGauss * DT;
		hW[idx+4*num] -= momentRateSlice[i*MOMSIZE+1] * factorGauss * DT;
		hW[idx+5*num] -= momentRateSlice[i*MOMSIZE+2] * factorGauss * DT;
		hW[idx+6*num] -= momentRateSlice[i*MOMSIZE+3] * factorGauss * DT;
		hW[idx+7*num] -= momentRateSlice[i*MOMSIZE+4] * factorGauss * DT;
		hW[idx+8*num] -= momentRateSlice[i*MOMSIZE+5] * factorGauss * DT;
	END_CALCULATE1D( ) 
...
__GLOBAL__
void addSource1( FLOAT * hW,  float * momentRateSlice, long long * srcIndex, int npts, int _nx_, int _ny_, int _nz_, float DT )
{
	...
	CALCULATE1D( i, 0, npts )
		...
		//V = Jac[idx] * DH * DH * DH;
		//V = -1.0 / V;
		hW[idx+3*num] -= momentRateSlice[i*MOMSIZE+0] * DT;
		hW[idx+4*num] -= momentRateSlice[i*MOMSIZE+1] * DT;
		hW[idx+5*num] -= momentRateSlice[i*MOMSIZE+2] * DT;
		hW[idx+6*num] -= momentRateSlice[i*MOMSIZE+3] * DT;
		hW[idx+7*num] -= momentRateSlice[i*MOMSIZE+4] * DT;
		hW[idx+8*num] -= momentRateSlice[i*MOMSIZE+5] * DT;
	END_CALCULATE1D( ) 
		...
		float * gaussFactor, int nGauss, int flagSurf, int * useMultiSource )
{
	...
	( hW, momentRateSlice, srcIndex, npts, _nx_, _ny_, _nz_, DT );
	CHECK( cudaDeviceSynchronize( ) );
