...
#define L2( W, VAR, VARSIZE, FB, SUB ) ( FB * ( W[INDEX_##SUB( i, j, k, + FB * 1 )+num*VAR] - W[index+num*VAR] ) * rDH )
#define L3( W, VAR, VARSIZE, FB, SUB ) ( FB * ( Cf1 * W[index+num*VAR] + Cf2 * W[INDEX_##SUB( i, j, k, + FB * 1 )+num*VAR] + Cf3 * W[INDEX_##SUB( i, j, k, + FB * 2 )+num*VAR] ) * rDH ) 
...
#define SUM9VAR( RET, A1, A2, A3, DT) \
RET = svadd_m(pg, A1,  A2);			  \
RET = svadd_m(pg, RET, A3);			  \
RET = svmul_m(pg, RET, DT);	
...
#define L( W, VAR, num, FB, SUB ) (  FB * ( af_1 * W[INDEX_##SUB( i, j, k, - FB * 1 )+num*VAR] + 	\
						 		   			af0  * W[index+num*VAR] +			  	  				\
						 		   			af1  * W[INDEX_##SUB( i, j, k, + FB * 1 )+num*VAR] + 	\
						 		   			af2  * W[INDEX_##SUB( i, j, k, + FB * 2 )+num*VAR] + 	\
						 		   			af3  * W[INDEX_##SUB( i, j, k, + FB * 3 )+num*VAR] ) * rDH )

#define L_SVE( RET, W, VAR, FB, SUB )								 \
Z_1 = svld1( pg, &(W[INDEX_##SUB( i, j, k, - FB * 1) + num * VAR])); \
Z0  = svld1( pg, &(W[index + num * VAR]));						   	 \
Z1  = svld1( pg, &(W[INDEX_##SUB( i, j, k, + FB * 1) + num * VAR])); \
Z2  = svld1( pg, &(W[INDEX_##SUB( i, j, k, + FB * 2) + num * VAR])); \
Z3  = svld1( pg, &(W[INDEX_##SUB( i, j, k, + FB * 3) + num * VAR])); \
																	 \
...
#define L_SME( W, VAR, FB, SUB, TILE )							 	 \
Z_1 = svld1( pg, &(W[INDEX_##SUB( i, j, k, - FB * 1) + num * VAR])); \
Z0  = svld1( pg, &(W[index + num * VAR]));						   	 \
Z1  = svld1( pg, &(W[INDEX_##SUB( i, j, k, + FB * 1) + num * VAR])); \
Z2  = svld1( pg, &(W[INDEX_##SUB( i, j, k, + FB * 2) + num * VAR])); \
Z3  = svld1( pg, &(W[INDEX_##SUB( i, j, k, + FB * 3) + num * VAR])); \
																	 \
pgIdx = svcmpeq(svptrue_b32(), svindex_s32(0, 1), VAR );		 	 \
coef = svdup_f32_x(pgIdx, rDH * af_1 * FB );						 \
svmopa_za32_m(TILE, pgIdx, pg, coef, Z_1);							 \
coef = svdup_f32_x(pgIdx, rDH * af0* FB );						     \
svmopa_za32_m(TILE, pgIdx, pg, coef, Z0 );						     \
coef = svdup_f32_x(pgIdx, rDH * af1  * FB );						 \
svmopa_za32_m(TILE, pgIdx, pg, coef, Z1 );							 \
coef = svdup_f32_x(pgIdx, rDH * af2  * FB );						 \
svmopa_za32_m(TILE, pgIdx, pg, coef, Z2 );							 \
coef = svdup_f32_x(pgIdx, rDH * af3  * FB );						 \
svmopa_za32_m(TILE, pgIdx, pg, coef, Z3 );							 
