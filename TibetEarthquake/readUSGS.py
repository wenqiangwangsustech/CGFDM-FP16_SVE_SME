...
for j in range( src_npts ):
	lonRectCenter  [j]    = usgsData[j, 1] + 0.1
...
frontT = np.max( HalfDurationMesh ) 
backT = np.max( HalfDurationMesh ) 
...
lonCoord = np.reshape(lonMesh, [NY, NX])
latCoord = np.reshape(latMesh, [NY, NX])
print("A: lon = %f, lat = %f" % (lonCoord[ 0,  0], latCoord[ 0,  0]))
print("B: lon = %f, lat = %f" % (lonCoord[ 0, -1], latCoord[ 0, -1]))
print("C: lon = %f, lat = %f" % (lonCoord[-1,  0], latCoord[-1,  0]))
print("D: lon = %f, lat = %f" % (lonCoord[-1, -1], latCoord[-1, -1]))
