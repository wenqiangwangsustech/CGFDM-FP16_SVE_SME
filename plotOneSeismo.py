...
fileName.append( params["out"] + "/station" ) 
	...
	for key in station.keys( ):
		...
		for iKey in Keys:
			if key == iKey:
				...
				i = stationKeyNum[key]
				if i == 0:
					plt.plot( t, U[version, i] / vmax * zoom, color = colors[version], ls = '-', label = NameMode[version], linewidth = lineWidth )
					plt.text( TMAX * 0.85, 0.05, "%.2fcm/s" % np.max( np.abs( U[version, i] * 100 ) ) )
					if len( Keys ) > 1:
						plt.legend( loc = 1 )
				else:
					plt.plot( t, U[version, i] / vmax * zoom, color = colors[version], ls = '-', linewidth = lineWidth )
					plt.text( TMAX * 0.85, 0.05, "%.2fcm/s" % np.max( np.abs( U[version, i] * 100 ) ) )
				break
...
for iKey in range( stationForDrawNum ):
	...
	if iKey == 0 :
		plt.plot( T, swcgfd3D_U[iKey] / vmax * zoom, color = "g", ls = '--', label = "SW-CGFDM3D", linewidth = lineWidth )
		if len( Keys ) > 1:
			plt.legend( loc = 1 )
	else:
		plt.plot( T, swcgfd3D_U[iKey] / vmax * zoom, color = "g", ls = '--', linewidth = lineWidth )
