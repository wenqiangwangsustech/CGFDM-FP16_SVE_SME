	...
	int xStartHalo, int VARSIZE, int _nx_, int _ny_, int _nz_, FLOAT_PACK_UNPACK_FUNC pack_unpack_func )
{
	...
	( wave, thisSendRecv, xStartHalo, X0, Y0, Z0, X, Y, Z );
	...
	int yStartHalo, int VARSIZE, int _nx_, int _ny_, int _nz_, FLOAT_PACK_UNPACK_FUNC pack_unpack_func )
{
	...
	blocks.z = ( Z0 + threads.z - 1 ) / threads.z;
	pack_unpack_func<<< blocks, threads >>>
	( wave, thisSendRecv, yStartHalo, X0, Y0, Z0, X, Y, Z );
	CHECK( cudaDeviceSynchronize( ) );
#else
	pack_unpack_func
	( wave, thisSendRecv, yStartHalo, X0, Y0, Z0, X, Y, Z );
	...
	int zStartHalo, int VARSIZE, int _nx_, int _ny_, int _nz_, FLOAT_PACK_UNPACK_FUNC pack_unpack_func )
{
	...
	blocks.z = ( Z0 + threads.z - 1 ) / threads.z;
	pack_unpack_func<<< blocks, threads >>>
	( wave, thisSendRecv, zStartHalo, X0, Y0, Z0, X, Y, Z );
	CHECK( cudaDeviceSynchronize( ) );
#else
	pack_unpack_func
	( wave, thisSendRecv, zStartHalo, X0, Y0, Z0, X, Y, Z );
...
//x direction data exchange
	xStartHalo = nx;
	if ( mpiNeighbor.X2 >= 0 ) FLOAT_PackUnpackX( wave, thisXSend2, xStartHalo, VARSIZE, _nx_, _ny_, _nz_,  FLOAT_packX );
	...
	xStartHalo = 0;
	if ( mpiNeighbor.X1 >= 0 ) FLOAT_PackUnpackX( wave, thisXRecv1, xStartHalo, VARSIZE, _nx_, _ny_, _nz_, FLOAT_unpackX );

	
	xStartHalo = HALO;
	if ( mpiNeighbor.X1 >= 0 ) FLOAT_PackUnpackX( wave, thisXSend1, xStartHalo, VARSIZE, _nx_, _ny_, _nz_,  FLOAT_packX );
	...
	xStartHalo = _nx;
	if ( mpiNeighbor.X2 >= 0 ) FLOAT_PackUnpackX( wave, thisXRecv2, xStartHalo, VARSIZE, _nx_, _ny_, _nz_, FLOAT_unpackX );

//y direction data exchange
	yStartHalo = ny;
	if ( mpiNeighbor.Y2 >= 0 ) FLOAT_PackUnpackY( wave, thisYSend2, yStartHalo, VARSIZE, _nx_, _ny_, _nz_,  FLOAT_packY );
	...
	yStartHalo = 0;
	if ( mpiNeighbor.Y1 >= 0 ) FLOAT_PackUnpackY( wave, thisYRecv1, yStartHalo, VARSIZE, _nx_, _ny_, _nz_, FLOAT_unpackY );

	
	yStartHalo = HALO;
	if ( mpiNeighbor.Y1 >= 0 ) FLOAT_PackUnpackY( wave, thisYSend1, yStartHalo, VARSIZE, _nx_, _ny_, _nz_,  FLOAT_packY );
	...
	yStartHalo = _ny;
	if ( mpiNeighbor.Y2 >= 0 ) FLOAT_PackUnpackY( wave, thisYRecv2, yStartHalo, VARSIZE, _nx_, _ny_, _nz_, FLOAT_unpackY );

//z direction data exchange
	zStartHalo = nz;
	if ( mpiNeighbor.Z2 >= 0 ) FLOAT_PackUnpackZ( wave, thisZSend2, zStartHalo, VARSIZE, _nx_, _ny_, _nz_,  FLOAT_packZ );
	...
	zStartHalo = 0;
	if ( mpiNeighbor.Z1 >= 0 ) FLOAT_PackUnpackZ( wave, thisZRecv1, zStartHalo, VARSIZE, _nx_, _ny_, _nz_, FLOAT_unpackZ );

	
	zStartHalo = HALO;
	if ( mpiNeighbor.Z1 >= 0 ) FLOAT_PackUnpackZ( wave, thisZSend1, zStartHalo, VARSIZE, _nx_, _ny_, _nz_,  FLOAT_packZ );
	...
	zStartHalo = _nz;
	if ( mpiNeighbor.Z2 >= 0 ) FLOAT_PackUnpackZ( wave, thisZRecv2, zStartHalo, VARSIZE, _nx_, _ny_, _nz_, FLOAT_unpackZ );
