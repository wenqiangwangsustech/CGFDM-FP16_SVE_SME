...
{																	
	...
	svbool_t pg = svwhilelt_b32( HALO, _nx );
	svint32_t offset;
	...
	CALCULATE3D_SVE( i, j, k, HALO, _nx, HALO, _ny, HALO, _nz )  
		...
		STORE_HW( h_W, index, WSIZE, 0,	TMP, Vx1 , Vx2 , Vx3 , DT);							
		STORE_HW( h_W, index, WSIZE, 1,	TMP, Vy1 , Vy2 , Vy3 , DT);							
		STORE_HW( h_W, index, WSIZE, 2,	TMP, Vz1 , Vz2 , Vz3 , DT);							
		STORE_HW( h_W, index, WSIZE, 3,	TMP, Txx1, Txx2, Txx3, DT);						
		STORE_HW( h_W, index, WSIZE, 4,	TMP, Tyy1, Tyy2, Tyy3, DT);						
		STORE_HW( h_W, index, WSIZE, 5,	TMP, Tzz1, Tzz2, Tzz3, DT);						
		STORE_HW( h_W, index, WSIZE, 6,	TMP, Txy1, Txy2, Txy3, DT);						
		STORE_HW( h_W, index, WSIZE, 7,	TMP, Txz1, Txz2, Txz3, DT);						
		STORE_HW( h_W, index, WSIZE, 8,	TMP, Tyz1, Tyz2, Tyz3, DT);						
	END_CALCULATE3D( )												
}			
