...
{																	
	...
	CALCULATE3D_SVE( i, j, k, HALO, _nx, HALO, _ny, HALO, _nz )  
...
#endif
 		offset = svindex_s32(0, CJMSIZE * sizeof(FLOAT));
		xi_x = svld1_gather_offset( pg, &(CJM[index*CJMSIZE+0]), offset ); 
		xi_y = svld1_gather_offset( pg, &(CJM[index*CJMSIZE+1]), offset ); 
		xi_z = svld1_gather_offset( pg, &(CJM[index*CJMSIZE+2]), offset );	
		et_x = svld1_gather_offset( pg, &(CJM[index*CJMSIZE+3]), offset ); 
		et_y = svld1_gather_offset( pg, &(CJM[index*CJMSIZE+4]), offset ); 
		et_z = svld1_gather_offset( pg, &(CJM[index*CJMSIZE+5]), offset );	
		zt_x = svld1_gather_offset( pg, &(CJM[index*CJMSIZE+6]), offset ); 
		zt_y = svld1_gather_offset( pg, &(CJM[index*CJMSIZE+7]), offset ); 
		zt_z = svld1_gather_offset( pg, &(CJM[index*CJMSIZE+8]), offset );	

		mu 		 = svld1_gather_offset( pg, &(CJM[index*CJMSIZE+10]), offset );
		lambda 	 = svld1_gather_offset( pg, &(CJM[index*CJMSIZE+11]), offset );	
		buoyancy = svld1_gather_offset( pg, &(CJM[index*CJMSIZE+12]), offset );
		...
		offset = svindex_s32(0, WSIZE * sizeof(FLOAT));
		
		svst1_scatter_offset(pg, &(h_W[index * WSIZE + 0]), offset, HW0);						
		svst1_scatter_offset(pg, &(h_W[index * WSIZE + 1]), offset, HW1);						
		svst1_scatter_offset(pg, &(h_W[index * WSIZE + 2]), offset, HW2);						
		svst1_scatter_offset(pg, &(h_W[index * WSIZE + 3]), offset, HW3);						
		svst1_scatter_offset(pg, &(h_W[index * WSIZE + 4]), offset, HW4);						
		svst1_scatter_offset(pg, &(h_W[index * WSIZE + 5]), offset, HW5);						
		svst1_scatter_offset(pg, &(h_W[index * WSIZE + 6]), offset, HW6);						
		svst1_scatter_offset(pg, &(h_W[index * WSIZE + 7]), offset, HW7);						
		svst1_scatter_offset(pg, &(h_W[index * WSIZE + 8]), offset, HW8);						
