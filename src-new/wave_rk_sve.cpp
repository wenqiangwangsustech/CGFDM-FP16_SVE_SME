...
#endif
	//float h_w, w, t_w, m_w;
	svfloat32_t h_w, w, t_w, m_w;
		
	svfloat32_t svbeta1  = svdup_n_f32(beta1);
	svfloat32_t svalpha2 = svdup_n_f32(alpha2);

	uint64_t len = svcntw();
	svbool_t pg;

	CALCULATE1D_SVE( i, len, 0, WStride )
		pg = svwhilelt_b32( (uint64_t)i, (uint64_t)WStride );
		h_w = svld1(pg, &(h_W[i]));
		m_w = svld1(pg, &(W[i]));
...
void wave_rk1( FLOAT * h_W, FLOAT * W, FLOAT * t_W, FLOAT * m_W, uint64_t WStride)
{
	...
	uint64_t len = svcntw();
	svbool_t pg;
	svfloat32_t svbeta2  = svdup_n_f32(beta2);
	svfloat32_t svalpha3 = svdup_n_f32(alpha3);
	CALCULATE1D_SVE( i, len, 0, WStride )
		pg = svwhilelt_b32( (uint64_t)i, (uint64_t)WStride );
		h_w = svld1(pg, &(h_W[i]));
		t_w = svld1(pg, &(t_W[i]));
...
void wave_rk2( FLOAT * h_W, FLOAT * W, FLOAT * t_W, FLOAT * m_W, uint64_t WStride )
{
	...
	uint64_t len = svcntw();
	svbool_t pg;
	svfloat32_t svbeta3 = svdup_n_f32(beta3);
	CALCULATE1D_SVE( i, len, 0, WStride )
		pg = svwhilelt_b32( (uint64_t)i, (uint64_t)WStride );
		h_w = svld1(pg, &(h_W[i]));
		t_w = svld1(pg, &(t_W[i]));
...
void wave_rk3( FLOAT * h_W, FLOAT * W, FLOAT * t_W, FLOAT * m_W, uint64_t WStride )
{
	//printf( "wave_rk3\n" );
#ifdef GPU_CUDA
	uint64_t long i = threadIdx.x + blockIdx.x * blockDim.x;
#else
	uint64_t i = 0;
#endif
	svfloat32_t h_w, w, t_w, m_w;
	uint64_t len = svcntw();
	svbool_t pg;
	svfloat32_t svbeta4  = svdup_n_f32(beta4);
	CALCULATE1D_SVE( i, len, 0, WStride )
		pg = svwhilelt_b32( (uint64_t)i, (uint64_t)WStride );
		h_w = svld1(pg, &(h_W[i]));
		t_w = svld1(pg, &(t_W[i]));
