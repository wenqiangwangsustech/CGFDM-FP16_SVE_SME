						...
						FLOAT * CJM, int it, int irk, float DT, float DH )
{
	...
	h_W[INDEX( S.X, S.Y, S.Z )+num*3] += value;
	h_W[INDEX( S.X, S.Y, S.Z )+num*4] += value;
	h_W[INDEX( S.X, S.Y, S.Z )+num*5] += value;
}
						 ...
						 FLOAT * CJM, int it, int iRK, float DT, float DH, int nGauss, float rickerfc )
{
	...
	CALCULATE3D( i, j, k, S.X - nGauss, S.X + nGauss + 1, S.Y - nGauss, S.Y + nGauss + 1, S.Z - nGauss, S.Z + nGauss + 1 )   
		...
		float jacb = CJM[INDEX( S.X, S.Y, S.Z ) + num * 9];
    	...
    	h_W[index+3*num] = ( (float)h_W[index+3*num] + value );
		h_W[index+4*num] = ( (float)h_W[index+4*num] + value );
		h_W[index+5*num] = ( (float)h_W[index+5*num] + value );
...
void loadPointSource( GRID grid, SOURCE S, FLOAT * h_W, FLOAT * CJM, int it, int irk, float DT, float DH, float rickerfc )
{
		 ...
		 S.Z >= HALO && S.Z < _nz )
	{
		load_smooth_source
		<<< blocks, threads >>>
		( S, h_W, _nx_, _ny_, _nz_, CJM, it, irk, DT, DH, nGauss, rickerfc );
	}
...
int _NX_,    int _NY_,	  int _NZ_ )
{
	...
	CALCULATE3D( i, j, k, HALO, _nx, HALO, _ny, HALO, _nz )   
		...
		W[index+3*num] = 1e19f * ( 1.0f / ( PI * a2 ) ) * exp( r2 / a2 );
		W[index+4*num] = 1e19f * ( 1.0f / ( PI * a2 ) ) * exp( r2 / a2 ); 
		W[index+5*num] = 1e19f * ( 1.0f / ( PI * a2 ) ) * exp( r2 / a2 ); 
