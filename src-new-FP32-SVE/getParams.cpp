...
void getParams( PARAMS * params)
{
	...
	if (item = cJSON_GetObjectItem(object, "Non-Uniform"))
		params->Non_Uniform= item->valueint;

	if (item = cJSON_GetObjectItem(object, "nxl"))
		params->nxl = item->valueint;
	if (item = cJSON_GetObjectItem(object, "nxr"))
		params->nxr = item->valueint;

	if (item = cJSON_GetObjectItem(object, "nyl"))
		params->nyl = item->valueint;
	if (item = cJSON_GetObjectItem(object, "nyr"))
		params->nyr = item->valueint;

	if (item = cJSON_GetObjectItem(object, "nzl"))
		params->nzl = item->valueint;
	if (item = cJSON_GetObjectItem(object, "nzr"))
		params->nzr = item->valueint;
