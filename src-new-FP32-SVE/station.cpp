...
void storage_station( int stationNum, STATION station, FLOAT * W, int _nx_, int _ny_, int _nz_, int NT, int it )
{
	...
	CALCULATE1D( i, 0, stationNum ) 
		...
		station.wave[pos*WSIZE+0] = (float)W[index+num*0] * C1;
		station.wave[pos*WSIZE+1] = (float)W[index+num*1] * C1;
		station.wave[pos*WSIZE+2] = (float)W[index+num*2] * C1;
		station.wave[pos*WSIZE+3] = (float)W[index+num*3] * C2;
		station.wave[pos*WSIZE+4] = (float)W[index+num*4] * C2;
		station.wave[pos*WSIZE+5] = (float)W[index+num*5] * C2;
		station.wave[pos*WSIZE+6] = (float)W[index+num*6] * C2;
		station.wave[pos*WSIZE+7] = (float)W[index+num*7] * C2;
		station.wave[pos*WSIZE+8] = (float)W[index+num*8] * C2;
