...
#dataFP32 = np.load("./numpyData/%s_%d.npy"%(var, it))
#data = np.abs( dataFP32 - data )
plt.pcolormesh( dataX, dataZ, data, vmax = vm, vmin = -vm, cmap = "seismic" )
#plt.pcolormesh( dataX, dataZ, data * 10, vmax = vm, vmin = 0, cmap = "Reds" )
...
plt.title( "%s: time = %.2fs" % ( mod, it * DT ), fontsize = 14 )
