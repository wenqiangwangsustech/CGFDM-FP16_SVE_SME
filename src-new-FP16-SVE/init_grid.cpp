...
#ifdef PML
	if ( (grid->nx < grid->nPML) || (grid->ny < grid->nPML) || (grid->nz < grid->nPML) )
	{
			
		if (thisRank == 0)
		{	
			printf("The grid in each oprocess can't be less than PML layers (nPML)\n" );
			MPI_Abort(MPI_COMM_WORLD, 110);
		}
	}
#endif

	if (params.Non_Uniform == 1)
	{
		if (thisRank == 0)
		{
			printf("========================================================\n" );
			printf("We are uising non-uniform partitioning scheme\n" );
			printf("========================================================\n" );
		}
		init_heter_grid( params, grid, thisMPICoord );
	}
}
