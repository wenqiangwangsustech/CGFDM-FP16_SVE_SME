...
zy_data = np.loadtxt("./constructFiniteFaultModel/zhangyong/model2.txt")
NT = len( zy_data[0,9:] )
...
for j in range( depthPoint ):
	for i in range( lengthPoint ):
		...
		zy_slip_rate   [index] = zy_data[pos,9:]
'''
'''
...
sourceFileName = "source/" + "Zhangyong2.bin"
...
fig = plt.figure( 13)
plt.pcolormesh( XCoord, YCoord, dataSlipRate[:,:,5], cmap = "seismic" )
plt.colorbar( )
plt.axis( "equal" )
